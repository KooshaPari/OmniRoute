...
describe("benchmark output trust boundary", () => {
  it("accepts a bounded filename under the canonical evidence root", async () => {
    ...
    const output = await prepareBenchmarkOutputPath("run-1.json", root);
    const trustedRoot = await import("node:fs/promises").then((fs) => fs.realpath(root));
    expect(output).toBe(path.join(trustedRoot, "run-1.json"));
  });
