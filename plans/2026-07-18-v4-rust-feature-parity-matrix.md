...
| CAP-10 | BFF proxy and KBridge lifecycle | **Live but bounded debt.** HTTP proxy streaming/rollout works; #340 tracks abort, timeout, headers, and SSE characterization. #392 tracks KBridge inflight ordering, deadlines, cancellation, and Windows transport. | **Absent.** Axum API is not implemented, so no Rust ingress lifecycle exists. | v4 BFF for #340/#392; `omniroute-api` for convergence | Prove HTTP cancellation/streaming and KBridge race/deadline behavior before replacing ingress. |
