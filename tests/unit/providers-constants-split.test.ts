...
// + helpers still exported), the spread-merge integrity (no loss/dup), and that
...
test("APIKEY_PROVIDERS merges the 6 family files without loss or dupes", async () => {
  const keys = Object.keys((P as Record<string, object>).APIKEY_PROVIDERS);
  assert.equal(new Set(keys).size, keys.length, "duplicate keys after spread-merge");
  // the merged object's entry-count equals the sum of the 6 semantic family files; families are a
  // strict partition (every provider in exactly one).
  ...
  assert.equal(famTotal, keys.length, "families must partition all API-key providers");
});
