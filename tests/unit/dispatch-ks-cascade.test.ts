...
import {
  ...
  getEdgeTier,
} from "../../open-sse/rpc/dispatchEdges";
import {
  syncDispatchToKillSwitch,
  syncDispatchToPerProviderKillSwitch,
...
describe("kill-switch cascade", { concurrency: 1 }, () => {
  ...
  it("granular cascade only downgrades overlapping-provider edges", () => {
    ...
    const result = syncDispatchToPerProviderKillSwitch([
    ...
    const result2 = syncDispatchToPerProviderKillSwitch([
  ...
  it("wildcard provider scope matches everything", () => {
    ...
    syncDispatchToPerProviderKillSwitch([{ provider: "gemini", isActive: true }]);
