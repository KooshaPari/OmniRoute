...
describe("dispatch-tier e2e: full resolution pipeline", () => {
  ...
  it("registers, resolves tiers correctly", () => {
    ...
    reg("e2e.config", "T1");
    assert.equal(resolveTier("e2e.scoring", undefined, { cpuPressure: 0, memPressure: 0 }).tier, "T3");
    assert.equal(resolveTier("e2e.rateLimit", undefined, { cpuPressure: 0, memPressure: 0 }).tier, "T2");
    assert.equal(resolveTier("e2e.config", undefined, { cpuPressure: 0, memPressure: 0 }).tier, "T1");
  });
  ...
  it("kill-switch forces all to T1", () => {
    ...
    assert.equal(resolveTier("e2e.ks1", undefined, { cpuPressure: 0, memPressure: 0 }).tier, "T1");
    assert.equal(resolveTier("e2e.ks2", undefined, { cpuPressure: 0, memPressure: 0 }).tier, "T1");
    deactivateKillSwitchDegradation();
    assert.equal(isKillSwitchDegradationActive(), false);
    assert.equal(resolveTier("e2e.ks1", undefined, { cpuPressure: 0, memPressure: 0 }).tier, "T3");
    assert.equal(resolveTier("e2e.ks2", undefined, { cpuPressure: 0, memPressure: 0 }).tier, "T2");
  });
  ...
  it("full cycle: register, kill-switch, deactivate, reconcile", () => {
    reg("e2e.cycle", "T3");
    activateKillSwitchDegradation();
    assert.equal(resolveTier("e2e.cycle", undefined, { cpuPressure: 0, memPressure: 0 }).tier, "T1");
    deactivateKillSwitchDegradation();
