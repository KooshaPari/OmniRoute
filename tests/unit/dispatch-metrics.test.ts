...
test("recordTierDecision: increments counter for (old,new,reason,actor,edge)", () => {
  ...
  assert.match(text, /# TYPE dispatch_tier_decisions_total counter/);
  assert.match(
    text,
    /dispatch_tier_decisions_total\{[^}]*old_tier="T3"[^}]*new_tier="T1"[^}]*\} 2/
  );
});
...
test("recordTierDecision: buckets latency observations per (edge, new_tier)", () => {
  ...
  const text = renderPrometheusText();
  assert.match(text, /# TYPE dispatch_tier_decision_duration_us histogram/);
  // 5 µs falls in le="10" and up; 8500 µs falls only in le="10000" and +Inf
  // So le="10" should have count 1 (the 5µs sample).
  assert.match(text, /dispatch_tier_decision_duration_us_bucket\{[^}]*le="10"[^}]*\} 1/);
  assert.match(text, /dispatch_tier_decision_duration_us_bucket\{[^}]*le="\+Inf"[^}]*\} 2/);
});
...
test("renderPrometheusText: emits empty counters when no decisions recorded", () => {
  const text = renderPrometheusText();
  assert.match(text, /# HELP dispatch_tier_decisions_total/);
  assert.match(text, /# TYPE dispatch_tier_decisions_total counter/);
  // No data lines for the counter
  assert.doesNotMatch(text, /^dispatch_tier_decisions_total\{/m);
});
...
test("startMetricsEndpoint: serves /metrics text on a real port", async () => {
  ...
  if (server) {
    ...
    assert.match(text, /# HELP dispatch_tier_decisions_total/);
    assert.match(text, /dispatch_tier_decisions_total\{[^}]*edge="test\.edge"/);
    await server.close();
  }
  delete process.env.OMNIROUTE_METRICS_PORT;
  __resetMetricsForTests();
