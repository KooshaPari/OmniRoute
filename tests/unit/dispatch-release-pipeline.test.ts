...
test("release-pipeline: publish-ffi-packages.sh script exists", () => {
  ...
  // The script references directory names; npm publish reads scoped names from package.json
  assert.ok(content.includes("omniroute-ffi-") || content.includes("packages"),
    "publish script should reference FFI package directories"
  );
});
