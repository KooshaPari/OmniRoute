...
test("createProxyRegistrySchema accepts relay types and sources (schema gap-06)", async () => {
  ...
  for (const [type, source] of [
    ...
    const result = createProxyRegistrySchema.safeParse({
      ...
      type,
      host: "omniroute-relay.example.com",
      port: 443,
      source,
    ...
    assert.ok(
      result.success,
      `schema should accept type:${type} — errors: ${JSON.stringify("error" in result ? result.error : null)}`,
    );
    if (result.success) {
