/**
 * Tests for the polyglot FFI loader + ABI versioning (ADR-032, F4-F5).
 *
...
const { invoke } = await import("../../open-sse/rpc/polyglotEdges.ts");
const { __resetEdgeRegistryForTests } = await import("../../open-sse/rpc/polyglotEdges.ts");
...
test("invoke FFI edge returns FFI_NOT_AVAILABLE when crate is missing", async () => {
  const { registerEdge } = await import("../../open-sse/rpc/polyglotEdges.ts");
  registerEdge({
...
test("Rust signature-cache crate exposes the FFVv1 surface", async () => {
  ...
  assert.match(src, /pub extern "C" fn reasoning_replay_simd\(/, "missing reasoning_replay_simd");
});
