/**
 * Tests for the dispatch error taxonomy (ADR-032 § "Consequences").
 *
 * Covers:
 *   - isDispatchEdgeError discriminates by `code` field.
...
const { isDispatchEdgeError, degradationFallback } = await import("../../open-sse/rpc/errors.ts");
...
// ─── E1: isDispatchEdgeError ───────────────────────────────────────────────

test("isDispatchEdgeError recognizes errors with a code field", () => {
  const e = err("HTTP_500");
  assert.equal(isDispatchEdgeError(e), true);
});

test("isDispatchEdgeError rejects plain Error objects", () => {
  const e = new Error("nope");
  assert.equal(isDispatchEdgeError(e), false);
});

test("isDispatchEdgeError rejects null / undefined / primitives", () => {
  assert.equal(isDispatchEdgeError(null), false);
  assert.equal(isDispatchEdgeError(undefined), false);
  assert.equal(isDispatchEdgeError("string"), false);
  assert.equal(isDispatchEdgeError(42), false);
});
