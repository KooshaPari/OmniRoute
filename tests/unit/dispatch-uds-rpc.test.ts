/**
 * Tests for the dispatch UDS RPC server + client roundtrip (ADR-032, F1).
 *
 * Covers:
...
const tmp = mkdtempSync(join(tmpdir(), "dispatch-uds-"));
process.env.OMNIROUTE_UDS_SOCKET = join(tmp, "dispatch.sock");
process.env.DATA_DIR = tmp;

const { registerUdsHandler, start, stop, callHandlerInProcess, getDispatchUdsServer } = await import(
...
test("start() binds the UDS socket and accepts a TCP-via-UDS connection", async () => {
  ...
  await start();
  const srv = getDispatchUdsServer();
