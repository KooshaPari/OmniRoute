/**
 * Tests for the polyglot edge registry + invocation dispatch (ADR-032).
 *
 * Covers:
...
} = await import("../../open-sse/rpc/polyglotEdges.ts");
...
test("registerEdge rejects duplicate names", () => {
  reg("reg.dup", "T1");
  assert.throws(() => reg("reg.dup", "T1"), /already registered/);
});
