...
void test("otelBridge: recordTierDecision increments counter", () => {
  ...
  assert.ok(text.includes("dispatch_tier_decisions_total"));
...
void test("otelBridge: recordReconcileSweep records duration histogram", () => {
  ...
  const text = renderPrometheusText();
  assert.ok(text.includes("dispatch_tier_decision_duration_us"));
  assert.ok(text.includes("dispatch_tier_decision_duration_us_bucket"));
});
...
void test("otelBridge: renderPrometheusText returns valid OpenMetrics", () => {
  ...
  const text = renderPrometheusText();
  assert.ok(text.includes("# TYPE dispatch_tier_decisions_total counter"));
...
void test("otelBridge: listMetricSnapshots returns enriched snapshot objects", () => {
  ...
  const counter = snapshots.find((s) => s.name === "dispatch_tier_decisions_total");
