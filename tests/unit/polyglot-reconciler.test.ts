/**
 * Tests for polyglot tier reconciler boot + kill-switch aggregation (ADR-032).
 *
 * Validates:
 *   1. `startPolyglotReconciler()` returns a handle and doesn't crash.
 ...
 *   5. Env-gated opt-out via `OMNIROUTE_POLYGLOT_RECONCILER_ENABLED=false`.
 */

import assert from "node:assert/strict";
import { describe, it, afterEach } from "node:test";
import {
  startPolyglotReconciler,
  __resetPolyglotReconcilerForTests,
...
import { getEdge, __resetEdgeRegistryForTests, setEdgeTier } from "../../open-sse/rpc/polyglotEdges";
import { __resetKillSwitchBridgeForTests } from "../../open-sse/rpc/killSwitchBridge";

void describe("polyglot reconciler boot", () => {
  afterEach(() => {
    ...
    delete process.env.OMNIROUTE_POLYGLOT_RECONCILER_ENABLED;
  });

  it("startPolyglotReconciler returns a handle with stop/tickNow/running", () => {
    const h = startPolyglotReconciler({ tickMs: 60000 });
  ...
  it("startPolyglotReconciler is idempotent — second call returns same handle", () => {
    const h1 = startPolyglotReconciler({ tickMs: 60000 });
    const h2 = startPolyglotReconciler({ tickMs: 60000 });
  ...
  it("env OMNIROUTE_POLYGLOT_RECONCILER_ENABLED=false returns noop handle", () => {
    process.env.OMNIROUTE_POLYGLOT_RECONCILER_ENABLED = "false";
    __resetPolyglotReconcilerForTests();
    const h = startPolyglotReconciler();
  ...
  it("reconciler does not prevent the process from exiting", () => {
    const h = startPolyglotReconciler({ tickMs: 100000 });
  ...
  it("__resetPolyglotReconcilerForTests cleans up the cached handle", () => {
    const h1 = startPolyglotReconciler({ tickMs: 60000 });
    __resetPolyglotReconcilerForTests();
    const h2 = startPolyglotReconciler({ tickMs: 60000 });
...
void describe("polyglot reconciler kill-switch bridge", () => {
  afterEach(() => {
    __resetPolyglotReconcilerForTests();
    __resetEdgeRegistryForTests();
    __resetKillSwitchBridgeForTests();
