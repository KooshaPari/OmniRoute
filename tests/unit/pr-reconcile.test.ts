...
test("paginate returns a bounded page and rejects unsafe page inputs", () => {
  assert.deepEqual(core.paginate([1, 2, 3, 4, 5], 2, 2), [3, 4]);
  assert.deepEqual(core.paginate([1, 2], 0, 2), []);
  assert.deepEqual(core.paginate([1, 2], 1, 0), []);
});

test("capUtf8Bytes never splits a UTF-8 code point", () => {
  const capped = core.capUtf8Bytes("ab😀cd", 6);
  assert.equal(capped, "ab😀");
  assert.ok(Buffer.byteLength(capped, "utf8") <= 6);
});

test("compareHeadSha requires matching non-empty SHAs", () => {
  assert.equal(core.compareHeadSha("ABC123", "abc123"), true);
  assert.equal(core.compareHeadSha("", ""), false);
  assert.equal(core.compareHeadSha("abc", "def"), false);
});

test("isDuplicateEvent handles sets and arrays without mutation", () => {
  const seen = new Set(["delivery-1"]);
  assert.equal(core.isDuplicateEvent({ deliveryId: "delivery-1" }, seen), true);
  assert.equal(core.isDuplicateEvent("delivery-2", seen), false);
  assert.equal(core.isDuplicateEvent("delivery-1", ["delivery-1"]), true);
  assert.deepEqual([...seen], ["delivery-1"]);
});

test("getDispatchDecision applies policy, dedupe, and head gates", () => {
  assert.deepEqual(core.getDispatchDecision({ skip: true, skipReason: "draft" }), {
    dispatch: false,
    reason: "draft",
  });
  assert.deepEqual(core.getDispatchDecision({ duplicate: true }), {
    dispatch: false,
    reason: "duplicate_event",
  });
  assert.deepEqual(core.getDispatchDecision({ expectedHeadSha: "old", actualHeadSha: "new" }), {
    dispatch: false,
