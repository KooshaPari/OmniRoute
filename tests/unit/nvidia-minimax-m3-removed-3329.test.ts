...
// while other models on the same provider (e.g. `moonshotai/kimi-k2.6`) work.
// Advertising a model that 404s is a catalog bug; it is removed from the
...
test("nvidia tier does not advertise minimaxai/minimax-m3 (404 upstream) (#3329)", () => {
  ...
  // sanity: a known working sibling stays listed so the catalog is not empty
  assert.ok(ids.includes("moonshotai/kimi-k2.6"), "moonshotai/kimi-k2.6 stays available");
});
