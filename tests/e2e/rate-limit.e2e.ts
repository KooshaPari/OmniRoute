...
describe("E2E: checkRateLimit (limit=5, window=1s)", () => {
  it("allows requests within the limit", async () => {
    ...
    for (let i = 1; i <= 5; i++) {
      const result = await checkRateLimitWithRules(testKey, rules);
  ...
  it("rejects the 6th request in the same window", async () => {
    const rules = [{ limit: 5, window: WINDOW_S }];
    for (let i = 0; i < 5; i++) {
      await checkRateLimitWithRules(testKey, rules);
    }
    const result = await checkRateLimitWithRules(testKey, rules);
  ...
  it("isolates different keys", async () => {
    ...
    const rejected = await checkRateLimitWithRules(testKey, rules);
    expect(rejected.allowed).toBe(false);

    // Second key should still be fully available
    const other = await checkRateLimitWithRules(otherKey, rules);
  ...
  it("allows requests again after the window elapses", async () => {
    ...
    const allowed = await checkRateLimitWithRules(testKey, rules);
