...
export function normalizePetalsBaseUrl(value: string | null | undefined): string {
  ...
  if (/\/api\/v1\/generate$/i.test(normalized)) return normalized;
  if (/\/api\/v1$/i.test(normalized)) return `${normalized}/generate`;
  if (/\/api$/i.test(normalized)) return `${normalized}/v1/generate`;
