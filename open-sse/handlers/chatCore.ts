...
import {
  ...
  BifrostBackendExecutor,
} from "../executors/bifrost.ts";
import { runWithShadowSampler } from "../executors/bifrostShadow.ts";
import { shouldUseShadowDispatch, dispatchWithShadow } from "../services/trafficShadow.ts";
      ...
      try {
        ...
        const rawResult = await withRateLimit(
          ...
          async () => {
            ...
            // ── B6 Traffic shadow: fire-and-forget side-effect dispatch ──
            if (shouldUseShadowDispatch(provider)) {
              const shadowExecCreds = getExecutionCredentials();
              const bifrostExecutor = new BifrostBackendExecutor(provider, {} satisfies ProviderConfig);
              const shadowInput: ExecuteInput = {
                model: modelToCall,
                body: bodyToSend,
                stream: upstreamStream,
                credentials: shadowExecCreds,
                log,
                extendedContext,
              ...
              dispatchWithShadow(shadowInput, {
                legacyExecutor: executor,
                bifrostExecutor,
                provider,
                model: modelToCall,
              }).catch((shadowErr: unknown) => {
                log?.debug?.("B6_SHADOW", `Shadow dispatch failed (non-fatal): ${shadowErr instanceof Error ? shadowErr.message : String(shadowErr)}`);
              });
            }
