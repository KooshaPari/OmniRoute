/**
 * polyglotHotPath.ts — Production hot-path integration helper
 *
 * Provides single-call helpers that resolve the polyglot edge tier and invoke
...
/**
 * Chunk an SSE body through the polyglot-resolved T3 (FFI) or T2 (UDS RPC) or
...
/**
 * Score a batch of combo candidates through the polyglot-resolved binding.
...
/**
 * Consume tokens from the polyglot-resolved token-bucket binding.
 ...
 * Returns the total number of edges registered in the polyglot registry.
 */
export function polyglotEdgeCount(): number {
