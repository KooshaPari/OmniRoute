 ...
 *
 * Emits an audit event with action='dispatch.tier_change' every time
 ...
 * @see docs/adr/0032-dispatch-binding-tiers.md § "Audit-log hook"
 */
...
/**
 * Emit a `dispatch.tier_change` audit event.
 *
...
export function emitTierChangeAudit(rec: TierChangeRecord): void {
  try {
    ...
    if (typeof console !== "undefined") {
      console.warn("[dispatch] audit emit failed (non-fatal):", err);
    }
  }
}
