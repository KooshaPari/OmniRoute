...
> **Live counts (v3.8.40)**: providers 237 · MCP tools 104 · MCP scopes 31 · A2A skills 6 ·
...
**104 distinct tools** total (`countUniqueMcpTools`, `open-sse/mcp-server/toolCount.ts`): the runtime combines the base registry (`MCP_TOOLS` in `schemas/tools.ts`) with standalone memory, skill, agent-skill, GitHub-skill, pool, gamification, plugin, Notion, and Obsidian module sets, then deduplicates overlapping registrations. 3 transports (stdio / SSE / Streamable HTTP). Scoped auth (31 scopes — see `OMNIROUTE_MCP_SCOPES`), Zod schemas. See [`docs/frameworks/MCP-SERVER.md`](docs/frameworks/MCP-SERVER.md).
