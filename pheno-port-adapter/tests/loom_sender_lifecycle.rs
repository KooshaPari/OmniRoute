//! Loom test verifying the adapter's batched shutdown path is race-free.
//!
//! Exercises the read-modify-write pattern that production adapters
//! (TCP, Unix-domain socket) would use when flushing a queued batch on
//! shutdown: a `shutdown` flag is checked before each push, and an
//! `AtomicBool` is set to signal producer threads to drain. The loom
//! model checker enumerates every interleaving of the two `send` +
//! `shutdown` calls so we know the final state is reachable from any
//! ordering without a data race.
//!
...
/// Mock batch sender that mirrors the shape of the production adapters:
/// a shutdown flag gates writes, and the inner buffer is protected by
/// a `Mutex`.
#[derive(Default)]
struct BatchSender {
