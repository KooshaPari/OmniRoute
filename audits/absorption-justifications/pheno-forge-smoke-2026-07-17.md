...
| `scripts/run-smoke.sh` | Helper script: builds the bridge, sets `DYLD_LIBRARY_PATH` / `LD_LIBRARY_PATH`, runs the binary | **Absorbed as-is** at `crates/pheno-forge-smoke/scripts/run-smoke.sh`. Path resolves correctly because `pheno-cdylib-bridge` is a sibling crate under `crates/`. |
