...
|---|---|---|---|
| high | `scripts/pr-reconcile/cli.ts` exceeded the modularity target at 538 lines | resolved in `8f4ed8d2ee` | GitHub collection/pagination/current-head/event adapters now live in `scripts/pr-reconcile/github.ts`; `cli.ts` is 284 lines and `github.ts` is 272 lines |
