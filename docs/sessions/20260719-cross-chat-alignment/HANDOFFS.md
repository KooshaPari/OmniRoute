...
## 2026-08-01T08:30Z - qgate refresh at PR #485 head 35aa8bf52 (qgate_debt_plan)

- Current PR head `35aa8bf52df136220caa993b441d47ac56998b67`; qgate run `30688811301` is terminal `FAILURE` at coverage generation, while CI `TS/JS` run `30688811349` remains in progress in its unbounded `test` step after install/lint succeeded. The protected-context release gate is still red/incomplete.
- Rate-limit drift is reproduced: qgate logs report `setRateLimiterTestMode` missing, `_createRedisLogThrottleForTests` missing, `evictStaleRateLimitWindows` missing, and REDIS_URL fallback/strict-contract assertions failing. Treat this as one Keyv-vs-legacy rate-limiter contract migration PR, not CI suppression.
- Runner mismatch is reproduced: `tests/unit/auth/managementPassword.test.ts` crashes under the Node coverage glob with `@vitest/runner` `initSuite`/`getRunner` failures (qgate log around 07:03Z and 07:20Z). Keep runner partition and bounded test-command work ahead of feature fixes.
- Pricing/model drift is reproduced: `tests/unit/t12-pricing-updates.test.ts` reports actual `0.5` vs expected `0.27`; `tests/unit/t31-t33-t34-t38-model-specs.test.ts` reports actual context `1048576` vs expected `204800` and cap `200000` vs expected `131072`. Reconcile canonical catalog data and dated provider provenance in a separate PR.
- New exact qgate workflow debt: `tests/unit/qgate-workflow.test.ts` fails because `.github/workflows/qgate.yml` lacks expected `continue-on-error: true` on `Generate coverage (lcov)`. This is a narrow CI-only fix candidate, but should be coupled with bounded `TS/JS` timeout/partition work rather than masking baseline failures.
- Provider metadata/validator debt remains independent: `tests/unit/provider-nodes-validate-modelid.test.ts`, `tests/unit/provider-specific-data-schema.test.ts`, `tests/unit/provider-validation-branches.test.ts`, `tests/unit/provider-validation-hardening.test.ts`, and `tests/unit/provider-validation-specialty.test.ts` fail on endpoint fallback, schema limits, cookie/auth guidance, Nous Research, Command Code, and GitLawb/GMI semantics. Keep this as a dedicated provider-contract PR after rate-limit/catalog contracts.
- qgate coverage emitted despite test failures: Statements `79.41%` (`361961/455807`), branches `77.43%` (`61120/78927`), functions `84.83%` (`12796/15084`), lines `79.41%` (`361961/455807`). This is report-generation evidence only, not a passing gate.
- Final PR order: **P0 runner partition + bounded TS/JS command + qgate `continue-on-error` contract; P0 rate-limiter Keyv/legacy API migration; P0 pricing/model catalog source-of-truth reconciliation; P1 provider validator/metadata contracts; P2 unrelated baseline assertions (heap flag, memory/artifact probes).** This entry is append-only.
