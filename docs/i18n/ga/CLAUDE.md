...
Roimh brainse a bhaint nó PR a oscailt, rith seiceáil an bhun-glann (`AGENTS.md` → Sreaimh Oibre → "Seiceáil bun-glann"; tagraíonn scileanna an tionscadail dó mar `.agents/skills/_shared/base-green.md`). Ní mór don PR a osclaítear nuair atá an bun-fhoireann dearg ` bun-dearg faighte: #<issue>` a iompar ina chorp. Chun stát dearg carnaithe a dhraenáil (bun-fhoireann + PRanna dearga), úsáid an scile `/sweep-reds`.
