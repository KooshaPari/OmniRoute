...
do baseline 06-16 — o _buraco fast-gates_ e o _mutation-score-não-catraca_ — foram **fechadas**.
...
| --------------------------------- | ------------------------ | ----------------------------------------------------------------- | --------- | ------------------------------------------------------------------------ |
| **OWASP DSOMM** (5 níveis)        | L3→L4                    | **L4** em _Test Intensity_ e _Static Depth_; L3 sólido nas demais | ▲         | mutation-ratchet bloqueante + suíte determinística no gate-de-merge      |
...
| **SonarQube "Clean as You Code"** | Alinhado c/ ressalva     | Alinhado c/ ressalva                                              | =         | ressalva de _sprawl_ (~46+ gates) permanece — review de ROI pendente     |
...
**Hoje** `.github/workflows/quality.yml` roda, no job _Fast Quality Gates_: `typecheck:core`,
...
- **`semcheck.yaml` (camada LLM de drift semântico docs↔code) — REMOVIDO.** Estava **órfão**
  ...
  e a ressalva de _gate sprawl_ desaconselha adicionar um gate LLM advisory de custo recorrente.
