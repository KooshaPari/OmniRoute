...
| `SUBSTRATE_BIN`                                 | `cargo` (via `cargo run -q -p driver-cli --`)       | `src/lib/a2a/skills/agentDispatch.ts`                       | Path to the substrate `driver-cli` binary used by the `agent-dispatch` A2A skill. When unset, dispatches via `cargo run -q -p driver-cli --`.                          |
