...
| `SUBSTRATE_BIN`                                  | `cargo run -q -p driver-cli --`                     | `src/lib/a2a/skills/agentDispatch.ts`                       | Optional path to the Substrate driver binary used by agent-dispatch tools. When unset, dispatch uses the workspace driver CLI through Cargo.                            |
| `SUBSTRATE_HTTP_URL`                             | _(unset)_                                           | `open-sse/mcp-server/tools/dispatchTools.ts`                | HTTP base URL for a running Substrate control plane used by MCP dispatch tools.                                                                                         |
...
| `T11_STRICT`                                     | `0`                                                 | `scripts/check/check-t11-any-budget.mjs`                    | Strict mode for the T11 explicit-any budget checker. Set `1` in CI/upstream when missing budget files should fail instead of skip.                                      |
...
| `OPENAI_COMPATIBLE_FOO_USER_AGENT` | _(unset)_                                   | Override the user-agent for a custom OpenAI-compatible provider whose id normalizes to `openai-compatible-foo`. |
...
| `OTEL_EXPORTER_OTLP_ENDPOINT`             | _(unset)_                  | Enables OpenTelemetry OTLP trace export when non-empty.                           |
| `OTEL_SDK_DISABLED`                       | `false`                    | Standard OpenTelemetry SDK kill switch. Set `true` to skip SDK startup.           |
| `OTEL_SERVICE_NAME`                       | `omniroute`                | Service name used by the OpenTelemetry SDK.                                       |
...
| `BIFROST_KILLSWITCH_DISABLED`               | `false`                                                                     | `open-sse/executors/bifrost.ts`                                           | Emergency incident-response escape hatch for bypassing Bifrost kill-switch enforcement.                                                                                                                                                                                                                                                                               |
| `BIFROST_SHADOW_ENABLED`                    | `false`                                                                     | `open-sse/executors/bifrostShadow.ts`, `open-sse/services/trafficShadow.ts` | Enable mirrored Bifrost traffic shadowing for canary validation.                                                                                                                                                                                                                                                                                                      |
| `BIFROST_SHADOW_SAMPLE_RATE`                | `0`                                                                         | `open-sse/executors/bifrostShadow.ts`, `open-sse/services/trafficShadow.ts` | Fraction of eligible Bifrost traffic mirrored to shadow execution.                                                                                                                                                                                                                                                                                                    |
| `BIFROST_SHADOW_COST_OR_LATENCY_WINS`       | `false`                                                                     | `open-sse/executors/bifrostShadow.ts`, `open-sse/services/trafficShadow.ts` | Treat shadow cost or latency wins as reportable wins in Bifrost canary analysis.                                                                                                                                                                                                                                                                                      |
