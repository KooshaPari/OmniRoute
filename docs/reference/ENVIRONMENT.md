...
| `CIRCUIT_BREAKER_OPOSSUM_SHADOW`              | `0`     | `src/shared/utils/circuitBreaker.ts` | Feature flag: when `1`, runs a passive opossum circuit-breaker shadow alongside the primary breaker for migration telemetry. |
| `CIRCUIT_BREAKER_OPOSSUM_SHADOW_VERBOSE`      | `0`     | `src/shared/utils/circuitBreaker.ts` | Feature flag: when `1`, logs per-fire divergence details between primary and shadow breakers to stderr.               |
...
| `RATE_LIMITER_KEYV_URL`                        | `keyv://memory`          | `src/shared/utils/rateLimiter.ts`                                                       | Keyv connection URI for rate limit counters. Keep as `keyv://memory` (in-process default) or `keyv://sqlite:<path>` / `keyv://memory` alternatives for persistence behavior.                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `QUOTA_STORE_KV_URL`                        | `keyv-sqlite://default`                                                    | `src/lib/quota/storeFactory.ts`                                           | Keyv connection URI for `QUOTA_STORE_DRIVER=keyv`. Defaults to an embedded SQLite-backed file unless overridden.                                                                                                                                                                                                                                                                         |
