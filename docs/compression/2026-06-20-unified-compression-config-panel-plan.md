...
- `src/lib/db/migrations/116_compression_engines_map.sql` — backfill `engines` + `activeComboId`.
...
## Task 4: Migration 116 — backfill `engines` + `activeComboId`

**Files:**
- Create: `src/lib/db/migrations/116_compression_engines_map.sql`
...
test("migration backfills engines map from prior defaultMode + default combo", () => {
  ...
  // simulate a pre-116 install: master on, defaultMode 'standard', caveman enabled
...
- [ ] **Step 2: Run → FAIL.**
- [ ] **Step 3: Implement** the SQL migration (idempotent; the row backfill of derived `engines` is done in `normalizeCompressionSettings` read-path — the SQL only seeds `activeComboId` default and a marker). Migration `116_compression_engines_map.sql`:
...
- [ ] **Step 5: Commit** `git add src/lib/db/migrations/116_compression_engines_map.sql src/lib/db/compression.ts tests/unit/compression/compression-engines-map-migration.test.ts && git commit -m "feat(compression): persist+backfill engines map and activeComboId (migration 102)"`
...
2. **Modelo de dados Fase A** — `engines` map + `activeComboId` em `CompressionConfig`, com migração 116 + backfill (zero mudança de comportamento).
