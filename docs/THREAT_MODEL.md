...
│ │    ┌──────────────────────────────────────────────┐           │   │
│ │    │  src/proxy.ts (auth + tier routing)         │           │   │
│ │    │   ┌────────┐ ┌────────┐ ┌────────┐ ┌──────┐ │           │   │
...
`src/proxy.ts`. Anything past this boundary is fully authenticated
...
| **D**  | M — Image gen is $expensive          | Per-key cost-budget rules (`src/domain/costRules.ts`, added in [the MCP router substrate (per ADR-013 polyglot reuse via canonical ports)]) |
| **E**  | L                                    |                                                                                                                                             |
...
| MANAGEMENT (`/api/settings/*`, `/api/keys/*`, `/api/quota/*`, `/api/v1/management/*`)                                               | ~60   | Manage-scope + dashboard session + 2FA suggested           | See SECURITY.md for P0 items         |
