...
│ │    ┌──────────────────────────────────────────────┐           │   │
│ │    │  src/server/authz/pipeline.ts (auth + tier routing)    │           │   │
│ │    │   ┌────────┐ ┌────────┐ ┌────────┐ ┌──────┐ │           │   │
...
`src/server/authz/pipeline.ts`. Anything past this boundary is fully authenticated
...
| **D**  | M — Image gen is $expensive          | Per-key usage-cost enforcement in the usage + quota layers (`open-sse/services/usage.ts`, `open-sse/services/accountFallback.ts`) |
| **E**  | L                                    |                                                                                                                                   |
...
| ----------------------------------------------------------------------------------------------------------------------------------- | ----- | ---------------------------------------------------------- | ------------------------------------ |
| PUBLIC (`/api/health`, `/api/system/version`, `/_next/*`)                                                                           | 3     | No auth; rate limit; static response                       | DoS via flooding (mitigated at LB)   |
...
| MANAGEMENT (`/api/settings/*`, `/api/keys/*`, `/api/quota/*`, `/admin/*)                                                            | ~60   | Manage-scope + dashboard session + 2FA suggested           | See SECURITY.md for P0 items         |
