...
The v3.7.x → v3.8.0 cycle added zero-config auto routing, new providers, OAuth flows, deeper resilience, and a substantially expanded CLI. Headline features are listed below; full details follow later in this document and in the linked specs.
...
- **Cursor Pro plan usage** — quota and cycle data surfaced in the provider-limits dashboard
...
- **DeepSeek quota and limit monitoring** — daily/monthly usage exposed via the dashboard
- **Reset-aware routing strategy** — combos now prefer accounts whose quota window resets soonest
- **`fallbackDelayMs`** and **dynamic tool limit detection** — finer fallback timing plus per-provider tool-count limits
...
- **Per-provider 429 classification** plus the `useUpstream429BreakerHints` toggle — finer breaker behavior using upstream rate-limit hints
- **Model cooldowns dashboard** — observe per-model lockouts and re-enable them manually from the UI
- **MITM dynamic Linux cert detection** — works across Debian/Ubuntu, Fedora/RHEL, Arch, and other distributions
...
- **API Keys / Bearer keys with `manage` scope** — perform admin operations programmatically via the API
- **Combo target health analytics** and the **structured combo builder** — per-target health plus a UI builder for assembling `(provider, model, connection)` steps
- **GitLab Duo OAuth provider** — log in with GitLab credentials
- **Reasoning Replay Cache** — hybrid in-memory and SQLite persistence of reasoning traces
...
Manage AI provider connections: OAuth providers (Claude Code, Codex), API key providers (Groq, DeepSeek, OpenRouter), and free providers (Qoder, Kiro). Kiro accounts include credit balance tracking — remaining credits, total allowance, and renewal date are visible in Dashboard → Usage.
...
- **Resilience** — Rate limit persistence, circuit breaker tuning, auto-disable banned accounts, provider expiration monitoring, **Context Relay** handoff threshold and summary model configuration, per-provider 429 classification and the `useUpstream429BreakerHints` toggle, model cooldowns
...
- **Per-model visibility toggle** (eye icon) — Hidden models are grayed out and excluded from the `/v1/models` catalog
...
- Tauri 2 build packaging — hardened standalone bundle validation detects symlinked `node_modules` and rejects it before packaging, preventing a runtime dependency on the build machine (v2.5.5+)
...
The audit log has been expanded with cursor-based pagination, request context enrichment (request ID, user agent, IP), structured auth events, provider CRUD events with diff context, and SSRF-blocked validation logging. New events are emitted by `src/lib/compliance/providerAudit.ts`.
