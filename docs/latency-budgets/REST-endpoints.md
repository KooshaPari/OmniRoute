...
**Status:** Authoritative per-endpoint budget source with CI enforcement hooks
...
2. The tracked endpoint manifest in this document, evaluated by
   `tools/latency-budget-to-ci/budget.py` against collected spans
...
| Endpoint > budget p99 (but under hard cap) | Warning annotation, PR not blocked                   |
| Endpoint > 10% regression from main        | PR blocked unless label `latency-exception` applied  |
| Missing endpoint from trace                | Warning (may be a new endpoint without a baseline)   |
