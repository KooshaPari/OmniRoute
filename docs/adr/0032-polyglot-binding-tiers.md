# 0032 — Polyglot Binding Tiers (HTTP → UDS/RPC → FFI)

> Status: **Fully Shipped (F1–F5 + Production Wiring + CI Gate + npm Packages); F6 mock bridge complete, deferred to 2027 Q1 (Bifrost v1.0 GA)**
...
> [`bench-results/polyglot-tier-matrix-v2.json`](../../bench-results/polyglot-tier-matrix-v2.json).
...
The user directive (2026-07-04): _"consider polyglot with native ABI FFI or
similar direct bindings, or unix sockets/RPCs and others for hot modules/hot
paths and edges or other modules."_

The fleet already has the polyglot substrates to make this real:
...
- **Verdict**: the right default (T1) but not the right _only_.
...
- **Cons**: requires a per-edge mapping table; three binding mechanisms
  ...
  `napi-rs`).
- **Verdict**: the right answer when the fleet has polyglot substrates
...
- **Process proliferation** — T2 UDS adds one process per language
  surface; Mitigated by a single `omniroute-polyglot-host` supervisor
...
The following table is auto-generated by `benches/polyglot/matrix-generator.ts` from real
...
Full raw data at `bench-results/polyglot-tier-matrix.json` and
`bench-results/polyglot-tier-matrix.md`. Re-run with:
...
Raw data: [`bench-results/polyglot-tier-matrix-v2.json`](../../bench-results/polyglot-tier-matrix-v2.json).
...
npm run bench:polyglot      # TS baselines (Appendix A)
...
matrix to a first-class CI gate. Reads `bench-results/polyglot-tier-matrix.json`
...
`open-sse/rpc/audit.ts::emitTierChangeAudit()` emits a `polyglot.tier_change`
audit event for every edge flip with `action: "polyglot.tier_change"`,
`actor: "polyglot_reconciler"`, and `metadata.old_tier` / `new_tier` /
...
TS  :  tests 102+ pass 102+ fail 0 (12 polyglot files + 3 new: token-bucket, ks-cascade, tier-matrix)
...
node --import tsx/esm --test tests/unit/polyglot-{token-bucket-edges,ks-cascade,tier-matrix-verify,guardrails-edges}.test.ts
...
**Status:** Applied (F6 D-scaffold, E wires, F dashboards) · `npm run tier-matrix:verify` · $ cargo test && node polyglot-*.test.ts
...
Six new edges registered in `open-sse/rpc/polyglotEdges.ts` (from 17→23+):
...
| Recording rules             | `ops/monitoring/prometheus-rules.yaml`           | `polyglot_edges_at_capacity`, `polyglot_tier_flip_rate_5m`     |
| Alerts                      | `ops/monitoring/prometheus-alerts.yaml`          | `PolyglotTierDegraded`, `PolyglotDefaultCooldown`              |
| Grafana dashboard           | `ops/monitoring/grafana-dashboard-polyglot.json` | 3-panel: flips/sec gague + tier overlay + edge decay histogram |

**Metrics family** (exposed at `GET /metrics`):

```prometheus
# HELP polyglot_tier_decisions_total Edge tier decisions by reason
# TYPE polyglot_tier_decisions_total counter
polyglot_tier_decisions_total{old_tier="T3",new_tier="T2",reason="cpu_pressure",actor="reconciler"} 3

# HELP polyglot_reconcile_sweep_duration_milliseconds Duration of reconcile sweep (prometheus-style, not standard histogram — durations in seconds)
# TYPE polyglot_reconcile_sweep_duration_milliseconds histogram
polyglot_reconcile_sweep_duration_milliseconds_bucket{actor="reconciler",le="10"} 1
polyglot_reconcile_sweep_duration_milliseconds_bucket{actor="reconciler",le="10000"} 2
polyglot_reconcile_sweep_duration_milliseconds_count{actor="reconciler"} 2

# HELP polyglot_current_tier Current assigned edge tier per edge
# TYPE polyglot_current_tier gauge
polyglot_current_tier{edge_name="sse.chunk.sseStream"} 3
polyglot_current_tier{edge_name="guardrails.pii.anonymize"} 3
```
...
| ------------------------- | ------------------------------------------------------------------------- | -------- | ------------------------------------------------------------------ |
| `PolyglotTierDegraded`    | `increase(polyglot_tier_decisions_total{reason="cpu_pressure"}[5m]) > 30` | Warning  | More than 30 tier flips to CPU-pressure in 5 min → check host load |
| `PolyglotDefaultCooldown` | `time() - polyglot_last_metric_update > 120`                              | Critical | Reconciler stopped updating metrics for 2+ minutes                 |

### E.4 Tests

| Test file                                    | Tests | Verifies                                                                               |
| -------------------------------------------- | ----- | -------------------------------------------------------------------------------------- |
| `tests/unit/polyglot-expanded-edges.test.ts` | 6     | All 6 new edges register + respond to getEdgeTier                                      |
| `tests/unit/polyglot-monitoring.test.ts`     | 3     | Counter increment, histogram observe, /metrics format                                  |
| `tests/unit/polyglot-bifrost-bridge.test.ts` | 6     | Cargo.toml workspace member exists, Go source files exist, cross-compile for linux-x64 |
...
node --import tsx/esm --test tests/unit/polyglot-expanded-edges.test.ts

# F — Monitoring + metrics
node --import tsx/esm --test tests/unit/polyglot-monitoring.test.ts

# DevOps dashboards
# import ops/monitoring/grafana-dashboard-polyglot.json into Grafana
# apply ops/monitoring/prometheus-rules.yaml && ops/monitoring/prometheus-alerts.yaml

# Full sweep
node --import tsx/esm --test tests/unit/polyglot-*.test.ts
...
**Total:** 7 appendices (A–G) · 24 edges · 24 Rust tests · 7 FFI crates (incl. napi-rs) · 163+ TS tests · 3 production paths wired · napi-rs TypedArray ABI shipped · 0 doc-drift claims.

---

## Appendix F — Production Hot-Path Wiring (2026-07-08)

Polyglot binding tiers wired into the actual production request path via
`open-sse/rpc/polyglotHotPath.ts` (helper wrapping `polyglotEdges.ts` + `tierResolver.ts`).
...
uninterrupted. This is a **read-through advisory** pattern: polyglot binds compute, but
the original implementation always has the last word.

### Monitoring

`polyglot_tier_decisions_total{old_tier, new_tier, reason, actor="hot_path"}` counter
at `/metrics` (via `open-sse/rpc/metricsRoute.ts`) tracks every polyglot decision in
production.

### Rollback

```bash
# If polyglot edge causes issues, force all edges to T1 at boot:
export OMNIROUTE_POLYGLOT_RECONCILER_ENABLED=false
# Or per-edge:
...
node --import tsx/esm --test tests/unit/polyglot-typed-abi.test.ts
node --import tsx/esm --test tests/unit/polyglot-scoring-edges.test.ts
