...
- **Translation cost**: A small mapping layer is needed between OmniRoute's provider/model names and Bifrost's. Implemented in `open-sse/executors/bifrostProviderMap.ts`; keep future regression coverage aligned with `tests/unit/bifrost-backend.test.ts`.
