#!/usr/bin/env node
/**
 * matrix-generator.ts — Dispatch tier-verification matrix builder
 *
 ...
 * produces:
 *   1. bench-results/dispatch-tier-matrix.json  — machine-readable
 *   2. bench-results/dispatch-tier-matrix.md    — human-readable table
 *
 ...
 * Real measurements come from `benches/dispatch/*.bench.ts`.
 */
...
const MATRIX_JSON = resolve(RESULTS_DIR, "dispatch-tier-matrix.json");
const MATRIX_MD = resolve(RESULTS_DIR, "dispatch-tier-matrix.md");
...
function loadBenchResults(): { result: BenchResult; source: string }[] {
  const files = readdirSync(RESULTS_DIR).filter(
    (f) => f.endsWith(".json") && !f.startsWith("dispatch-tier-matrix")
  );
...
async function main() {
  ...
  const lines: string[] = [
    "# Dispatch Tier-Verification Matrix",
    "",
    `Auto-generated by \`benches/dispatch/matrix-generator.ts\` on ${new Date().toISOString()}`,
    "",
