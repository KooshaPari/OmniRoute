...
function isSchemaAlreadyApplied(
  ...
  switch (migration.version) {
    ...
    case "122":
      // virtual_keys renumbered 102 → 122 (collided with 102_fleet_nodes).
      // DBs that applied virtual_keys under the old 102 number should not re-run.
      return hasTable(db, "virtual_keys");
    case "123":
      // fleet_config renumbered 103 → 123 (collided with 103_cost_events).
      // DBs that applied fleet_config under the old 103 number should not re-run.
      return hasTable(db, "fleet_config");
    case "124":
      // traffic_shadow_log renumbered 104 → 124 (collided with 104_scaling_policies).
      // DBs that applied traffic_shadow_log under the old 104 number should not re-run.
      return hasTable(db, "traffic_shadow_log");
    case "125":
      // traffic_shadow_config renumbered 105 → 125 (collided with 105_alert_rules).
      // DBs that applied traffic_shadow_config under the old 105 number should not re-run.
