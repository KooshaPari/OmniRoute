...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // 8 MB per image — for no benefit. The cast is the same idiom the repo already
  // uses for this (src/app/api/files/[id]/content/route.ts, imageGeneration.ts):
  // Buffer is a valid BodyInit at runtime, but its generic ArrayBufferLike
  // parameter is not assignable to BodyInit in the type system.
